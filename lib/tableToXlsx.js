    ...
    for (let rIdx = 0; rIdx < table.rows.length; rIdx++) {
      ...
      for (let cIdx = 0; cIdx < table.rows[rIdx].length; cIdx++) {
        ...
        if (cellInfo.type === 'number') {
          cellInfo.valueText && !isNaN(cellInfo.valueText) ? cell.value(parseFloat(cellInfo.valueText)) : '';
          ...
          // https://github.com/dtjohnson/xlsx-populate#dates
           cellInfo.valueText && moment(cellInfo.valueText).isValid() ?  cell.value(moment(cellInfo.valueText).toDate()).style('numberFormat', 'yyyy-mm-dd') :'';

        } else if (cellInfo.type === 'datetime') {
          cellInfo.valueText && moment(cellInfo.valueText).isValid() ?  cell.value(moment(cellInfo.valueText).toDate()).style('numberFormat', 'yyyy-mm-dd h:mm:ss') : '';
