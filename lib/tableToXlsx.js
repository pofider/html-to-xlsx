    ...
    for (let rIdx = 0; rIdx < table.rows.length; rIdx++) {
      ...
      for (let cIdx = 0; cIdx < table.rows[rIdx].length; cIdx++) {
        ...
        if (cellInfo.type === 'number') {
          cell.value((cellInfo.valueText && !isNaN(cellInfo.valueText) ? parseFloat(cellInfo.valueText) : ''))
          ...
          cell.value(cellInfo.valueText && moment(cellInfo.valueText).isValid() ? moment(cellInfo.valueText).toDate() : '').style('numberFormat', 'yyyy-mm-dd')
        } else if (cellInfo.type === 'datetime') {
          cell.value(cellInfo.valueText && moment(cellInfo.valueText).isValid() ? moment(cellInfo.valueText).toDate() : '').style('numberFormat', 'yyyy-mm-dd h:mm:ss')
