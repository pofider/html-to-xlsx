...
function tableToXlsx(table, id, cb) {
    ...
    for (var i = 0; i < table.rows.length; i++) {
        ...
        for (var j = 0; j < table.rows[i].length; j++) {
            if (curr_row > 30) {
              console.log("Checking " + i + " " + j, table.rows[i][j])
            }
            ...
            sheet1.set(curr_col + 1, i + 1, cell.value);
            sheet1.align(curr_col + 1, i + 1, cell.horizontalAlign);
            sheet1.valign(curr_col + 1, i + 1, cell.verticalAlign === "middle" ? "center" : cell.verticalAlign);

            if (isColorDefined(cell.backgroundColor)) {
                sheet1.fill(curr_col + 1, i + 1, {
                    type: 'solid',
                    fgColor: 'FF' + rgbToHex(cell.backgroundColor),
                    bgColor: '64'
                });
            }
            sheet1.font(curr_col + 1, i + 1, {
                family: '3',
                scheme: 'minor',
            ...
            if (cell.rowspan > 1) {
                sheet1.merge({col: curr_col+1, row: curr_row+1},{col: curr_col+1, row: curr_row+cell.rowspan});
                row_offset.push({col: j, stop: curr_row+cell.rowspan});
