...
function convert(html, cb) {
  ...
  if (options.strategy === 'phantom-server')
    currentStrategy = require('./serverStrategy.js');

  if (options.strategy === 'dedicated-process')
    currentStrategy = require('./dedicatedProcessStrategy.js');

  convert.currentStrategy = currentStrategy;

  if (currentStrategy) {
    return currentStrategy(options, html, id, icb);
  }
...
function tableToXlsx(table, id, cb) {
  ...
  for (var i = 0; i < table.rows.length; i++) {
    ...
    row_offset.forEach(function(item) {
    ...
    for (var j = 0; j < table.rows[i].length; j++) {
      ...
      sheet1.wrap(tmpCol, curr_row + 1, cell.wrapText === 'scroll');
      ...
      if (cell.rowspan > 1) {
        ...
        sheet1.merge(
          { col: tmpCol, row: curr_row + 1 },
          { col: tmpCol + coloffset, row: curr_row + cell.rowspan }
        );

        // store the rowspan for later use to shift over the column starting point
        tmp_offsets.push({
          col: tmpCol,
          stop: curr_row + cell.rowspan,
      ...
      if (cell.colspan > 1 && cell.rowspan === 1) {
        ...
        sheet1.merge(
          { col: tmpCol, row: curr_row + 1 },
          { col: tmpCol + coloffset, row: curr_row + 1 }
        );
...
module.exports = function(opt) {
  ...
  convert.kill = function() {
    if (
      convert.currentStrategy &&
      typeof convert.currentStrategy.kill === 'function'
    ) {
      convert.currentStrategy.kill();
    }
  };
