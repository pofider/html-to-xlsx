...
module.exports = (opt = {}) => {
  ...
  async function convert (html, convertOptions = {}) {
    ...
    if (convertOptions.legacy === true) {
      stream = await tableToXlsx.legacy(options, Array.isArray(tables) ? tables[0] : tables, id, convertOptions)
    } else {
      stream = await tableToXlsx(options, tables, id, convertOptions)
    }
